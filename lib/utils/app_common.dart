...
ReadMoreText readMoreTextWidget(String data, {
...
void doIfLogin({required VoidCallback onLoggedIn}) {
  if (isLoggedIn.value) {
    ...
    Get.to(() => SignInScreen(), arguments: false)?.then((value) {
      if (isLoggedIn.value) {
        onLoggedIn.call();
      }
    });
...
Widget rentAndPaidButton(
    {EdgeInsets? margin, required bool isTrailer, required int planId, required VoidCallback callBack, required int requiredPlanLevel, required btnText, required discountPrice, required discount,}) {
  return AppButton(
    width: double.infinity,
    color: rentedColor,
    hoverColor: appColorPrimary,
  ...
  if (isLoggedIn.value) {
    if (planId == 0 && planLevel == 0 && isFromSubscribeCard) {
      ...
      if (videoAccess == MovieAccess.freeAccess && isSupportedDevice.value) {
        ...
        } else if (videoAccess == MovieAccess.payPerView) {} else {
          callBack.call();
        }
      }
    }
