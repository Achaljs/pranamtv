...
class CoreServiceApis {
  ...
  static Future<SearchResponse> getSearchDetails({required String search}) async {
    ...
    return SearchResponse.fromJson(
      await handleResponse(
        await buildHttpResponse(
          getEndPoint(endPoint: APIEndPoints.getSearchMovie),
          method: HttpMethodType.GET,
        ),
      ),
    );
  ...
  static Future<List<NewContests>> getContestsList() async {
    final url = Uri.parse('https://pranamtv.in/api/contests');
    final response = await http.get(url);

    if (response.statusCode == 200) {
      final jsonData = json.decode(response.body);

      final contestModel = ContestModel.fromJson(jsonData);
      return contestModel.data?.newContests ?? [];
    } else {
