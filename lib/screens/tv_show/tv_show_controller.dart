...
class TvShowController extends GetxController {
  ...
  void download() {
    if (selectedEpisode.value.downloadQuality.length > 1) {
      selectedEpisode.value.downloadQuality.removeWhere((element) => element.type != URLType.url && element.type != URLType.local);
    }
    Get.bottomSheet(
      isDismissible: true,
  ...
  Future<void> checkIfAlreadyDownloaded() async {
    ...
    if (selectedEpisode.value.downloadQuality.length == 1 &&
        selectedEpisode.value.downloadQuality.first.quality == 'default' &&
        (selectedEpisode.value.downloadType != URLType.url && selectedEpisode.value.downloadType != URLType.local)) {
      showDownload(false);
    }

    bool downloaded = await FileStorage().checkIfAlreadyDownloaded(videoId: selectedEpisode.value.id);
